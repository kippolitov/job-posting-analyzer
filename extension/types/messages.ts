 ...
 *
 * trigger "navigation": the active tab navigated or the user switched tabs —
 * any prior activeTab grant is dead, so the panel must drop stale state.
...
export interface ActiveTabChangedMessage {
  ...
  tabId: number | null;
  trigger: "navigation" | "action-click";
}
