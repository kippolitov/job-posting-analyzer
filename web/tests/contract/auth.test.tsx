...
describe("auth contract (contracts/web-auth.md, spec US1 scenario 5)", () => {
  ...
  it("signed-out: shows the data-practices disclosure with working Privacy Policy / Terms links before sign-in (docs/compliance/prominent-disclosure.md)", async () => {
    clearSession();
    render(<App />);

    await screen.findByText(/signing in creates a free account/i);
    expect(
  ...
  it("403 NOT_AUTHORIZED (unverified email) surfaces the server's plain-language message", async () => {
    ...
    api.failNext("jobs", {
      ...
      code: "NOT_AUTHORIZED",
      message:
        "Sign-in requires a verified Google email address. Verify your email in your Google Account settings, then try again.",
    });
